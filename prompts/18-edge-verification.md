...
**Verify**: `grep -rhE "P-[0-9A-Z]{6}\?" vault/Family_Tree*.md | grep -v "^>" | grep -oE "P-[0-9A-Z]{6}\?" | wc -l` before and after each iteration; log the delta. ⚠ **The `grep -v "^>"` is REQUIRED and this prompt was missing it until 17 AUG 2026**: `route_digest.py` blockquotes entry text at the head of every `Family_Tree` file, so the naive form counts every `?` token the digest quotes a second time (measured that day: **334 naive vs 324 tracked**). The offset largely cancels in a before/after DELTA, which is why it survived unnoticed here, but it does NOT cancel against the tracked baseline that `21-session-start` and `23-session-review` record — so the same vault read two different "current" values depending on which prompt asked. Use the form above everywhere. The **floor is not 0** — it is the sum of edges that *cannot* be confirmed away: (a) held-out conflations (`flags: dup` / known merge-pending); (b) FS-GAP edges (correct but FS-absent — they belong to prompt-17, not here); (c) CONTRADICTED edges awaiting operator resolution; (d) edges on `life_status: living`/`unknown` (privacy-skipped). ⭐ **Add (e): edges whose FAR END has no FS profile at all (`fs: TBD`)** — no FS walk can clear those, so they are an existence probe owed first, not confirmable edge work; on one vault they were 59% of the total. Report the breakdown so the convergence target is the *confirmable* remainder, which goes to 0.

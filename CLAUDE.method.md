...
- ⭐ **A ZERO, AN EMPTY RESULT OR A UNIFORM ERROR IS A CLAIM ABOUT THE INSTRUMENT UNTIL ITS ROWS HAVE BEEN READ.** Before explaining any observed symptom — a gate finding, a count that moved, a sweep that returns nothing, a script that misbehaves — write four lines and run the fourth: `Symptom` (what was observed, verbatim), `H0` (the instrument is wrong, or the behaviour is correct and my model of it is not), `H1` (the mechanism actually suspected, named down to a function or a regex), `Discriminator` (the cheapest READ-ONLY check that separates them). Say which survived, and how; then edit. **H0 is not a formality** — it is the line every case-law incident below skipped. ⚠ **A count put in front of the operator is re-derived through the owning module's own parser**, never an ad-hoc chunker. ⚠ **A detector's finding is not a finding until its rows have been read**, and its ZERO is weaker still: a zero states what the detector can SEE. ⛔ **Uniform failures across a sweep are an AUTH or instrument diagnosis first and a data one second** — identical 503s or identical empties mean probe with a known-good fetch BEFORE a single negative result is logged, because "no records" has three explanations (not authenticated, wrong query, records absent) before it has one. ⚠ **An absence is evidence only when the channel was alive to carry it** — a relay that dies with what it reports on, a listing that ended early, a dropped connection: silence from a dead channel and silence from an empty corpus look identical, and only one is a finding. Where the question is about what happens as something dies, ask what reached durable storage afterwards instead. ⛔ **Destroying state is not a diagnostic step**: `scripts/guard_destructive.py` (PreToolUse) refuses `git reset --hard` / `clean -f` / `checkout --` / `restore` **when they target a VAULT** (the framework repo has a remote; the vault has no copy but the working one), in-place stream edits and truncating redirects over vault Markdown, and hand edits of the store-owned registers. ⚠ It reads the hook's `cwd`, follows a `cd` or a `-C`, and **fails safe**: with no `cwd` to judge by it refuses. A refusal is a prompt to ask the operator, never to find a variant that slips past it. (case law: § "A zero is a claim about the instrument")
...
- ⛔⛔ **THE WORKED EXAMPLE IS WHERE VAULT DATA CROSSES INTO THE PUBLIC REPO.** CLAUDE.md already forbids real names in the framework repo; the rule that keeps being broken is narrower and worth naming. **A docstring, commit message or test fixture that explains WHY a script exists is the one place a real person gets written down**, because the incident that motivated the code IS a real row: the surname, its `P-` id and its ARKs are exactly what makes the explanation convincing. **Twice in one sitting** (24 AUG 2026) a new script and its test carried live surnames into `scripts/`; `privacy-audit-repo` caught both before commit. ⚠ **The measurement survives without the names** — write "one woman sat in four live questions", never who. ⭐ **The vault's own log is the right home for the named version**, and the cross-reference runs that way only: framework -> "see the vault log", never the reverse. ⚠ **Run `./scripts/privacy-audit-repo` (it is RUBY, never `bash`) after writing any new script's docstring, not just before pushing** — and only `privacy-audit-repo: ok` is a pass. (case law: § "Style")
...
This section describes the **`narrative` person model**: many people per lineage file, each a `- meta:` entry. The framework default is the **`file`** model (one person file, YAML frontmatter); both encode the same `PersonRecord` fields and are inter-convertible (`scripts/convert_person_model.py`; runbook [workflows/switch-person-model.md](workflows/switch-person-model.md)). **All record-consuming tooling goes through the model-agnostic seam `scripts/person_store.py`** (`iter_people`/`write_person`). ⛔⛔ **AND THAT INCLUDES ONE-OFF ANALYSIS — NEVER HAND-ROLL AN ENTRY CHUNKER.** Sanctioned BODY bullets (`- **Sources**`, `- **Prior work**`, `- **MINTED …**`) are indistinguishable from an entry header under a naive `^\s*[-*]*\s*\*\*` rule, so an ad-hoc chunker truncates every entry at its own Sources bullet — and it does not fail quietly, it **MISFILES the dropped entry's text onto whoever precedes it**. Three incidents in two sittings (24-25 AUG 2026): a wrong shared-parent claim published to the register, a man's own wife wired as his mother, and a census population reported to the operator as **195 when it was 4**. ⭐ Readers that already exist: `person_store.iter_people`, `harvest_sources.entry_blocks_with_ids`, `extension_frontier.rows_with_bodies`. ⚠ A name is prose in many entries and an id in exactly one: **match on the id**, and read an entry's id from the meta line IMMEDIATELY under its header, never with a pattern allowed to cross a newline. The narratives are the single source of truth (`Person_Index.md` is retired); the gen-sorted roster is generated on demand: `python3 scripts/gen_person_index.py --write /tmp/roster.md` (also `--integrity`, `--gap-report`). **v3 grammar = a VALID YAML flow-mapping**, parseable by `yaml.safe_load` and by `gen_person_index._parse_flow_mapping`. Template: [vault-template/templates/person_narrative.md](vault-template/templates/person_narrative.md).
...
- `profile_status` — `stub` / `partial` / `complete`, orthogonal to `evidence_tier`. `complete` = a `- **Sources**` bullet citing EITHER (a) independent primary records (`host:locator`) OR (b) **scholarly apparatus** (FMG Medlands, Richardson, Complete Peerage, ODNB with DOI, the Henry Project, MGH and named chronicles, Great Migration, NEHGR / TAG, published town Vital Records), with page or section refs. User trees earn nothing under either limb. This does not change the ARK metric; the census splits 0-ARK structural entries into **`BOOK_SOURCED`** (cites apparatus: finished work) vs **`UNCITED`** (no citation: the real worklist; its route is a library pass, not an FS harvest). ⛔⛔ **NAMING A WORK YOU HAVE NOT READ MOVES THE ROW.** The BOOK_SOURCED test is a TEXT match on the entry, so a ROUTE suggestion written in citation form ("check the Visitation of Dorset") flips an UNCITED person into "documented as well as it ever will be" — measured 25 AUG 2026, one row, caught by `census_diff` in the same precheck. ⭐ **Write the route as a CLASS** ("the county's printed visitation", "the parish registers") and keep titles for the `Sources` bullet. Same shape as the locator rule that writing ABOUT a source credits it, and it bites harder here because the whole point of the split is to separate "nobody has documented this person" from "finished work". (case law: § "Person entry meta block")
...
- `adjudicated` — far-end ids of `?` edges WALKED AND JUDGED with the `?` kept deliberately (`adjudicated: '[P-G4C4Z5]'`); this is what makes such work a countable disposition. Write it only for an edge actually read and judged. ⛔ Never as a token suffix (`P-XXXXXX?!` is destroyed by the next `build_edges --apply`; a sibling key survives by construction). `ADJUDICATED_STALE` (advisory, baseline 0) flags a note that outlived its `?`; a stale entry HIDES a real candidate. (case law: § "Person entry meta block")
  - ⭐ `adjudicated_why`: **`fs-gap`** / **`hedge`** / **`contradicted`** / **`privacy`** / **`no-second-parent`** / **`unnamed-in-record`**; missing = `ADJUDICATED_UNEXPLAINED` (advisory). **Only `fs-gap` with a live FS PID at the far end is ever re-offered** (a labelled RE-CHECK row, ranked last); hedges and contradictions expire on EVENTS tracked as Open_Questions, not a clock. The last TWO are parent-ABSENCE values: each retires a `HALF_WIRED_PARENT` row and stands ALONE with no `adjudicated` list. ⚠⚠ **They are NOT synonyms, and Q322 is why** (operator, 24 AUG 2026): **`no-second-parent`** is an ANCESTRY statement ("one named parent is the whole truth"), **`unnamed-in-record`** a SOURCE statement ("the second parent existed and is unnamed in every record read"). One value carrying both meanings had the vault ruling opposite ways EIGHT DAYS APART on two identical Hingham cases. ⭐ **Ask `person_store.declares_parent_absence()` when you mean "retired from the worklist"** (the gate, the lane and `banked_parents_settled` all do); **read the TOKEN when you mean the ancestry claim** — a pedigree export or a society line must not treat `unnamed-in-record` as a terminus. The key is multi-valued (bare scalar `fs-gap` or quoted list `'[fs-gap, unnamed-in-record]'`): **read with `person_store.adjudicated_why_values()`, never a regex** (unknown tokens dropped). ⛔ Declare either only from a RECORD or named authority, never the graph (closing a parent edge from a spouse edge picks the wrong woman whenever there was a remarriage). ⛔⛔ **And declare a parent ABSENCE only when NO NAMED RECORD ROUTE SURVIVES**: *"unnamed in the records I have READ"* is not *"unnamed in the records that EXIST"*. An unlocated tradition ("prob. from the west of England") admits the declaration; a LOCATED parish register still unworked does NOT — that row is work, and retiring it drops it for ever. This test is what made two rulings eight days apart look contradictory when both outcomes were right (Q322, settled 24 AUG 2026). (pinned: `scripts/test_adjudicated_why.py`)

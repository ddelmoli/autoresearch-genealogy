...
def hist(*sittings):
    ...
    return [{"date": "2026-08-01", "lane": "IMPROVE", "outcome": "hit", "session": s}
            for s in sittings]
...
class TestCooling(unittest.TestCase):
    def test_unstamped_row_is_not_cooling(self):
        ...
        self.assertEqual(sp.cooling(st, "IMPROVE", "P-00001"), (False, None))

    def test_row_stamped_this_sitting_is_cooling(self):
        st = {"history": hist(1, 2, 3), "offered": {"IMPROVE": {"P-00001": "S3"}}}
        is_cool, since = sp.cooling(st, "IMPROVE", "P-00001")
    ...
    def test_cooldown_expires_after_OFFER_COOLDOWN_sittings(self):
        ...
        st = {"history": hist(1, 2, 3, 4), "offered": {"IMPROVE": {"P-00001": "S1"}}}
        is_cool, since = sp.cooling(st, "IMPROVE", "P-00001")
    ...
    def test_still_cooling_one_sitting_before_expiry(self):
        st = {"history": hist(1, 2, 3), "offered": {"IMPROVE": {"P-00001": "S1"}}}
        self.assertTrue(sp.cooling(st, "IMPROVE", "P-00001")[0])
    ...
    def test_unknown_stamp_reads_cold_not_error(self):
        ...
        st = {"history": hist(5, 6), "offered": {"IMPROVE": {"P-00001": "S1"}}}
        self.assertEqual(sp.cooling(st, "IMPROVE", "P-00001"), (False, None))

    def test_cooldown_is_PER_LANE(self):
        """Offering someone in IMPROVE must not cool them in EXPAND -- different work."""
        st = {"history": hist(1), "offered": {"IMPROVE": {"P-00001": "S1"}}}
        self.assertTrue(sp.cooling(st, "IMPROVE", "P-00001")[0])
    ...
    def test_keyed_on_vault_id_not_external_pid(self):
        ...
        st = {"history": hist(1), "offered": {"IMPROVE": {"XXXX-XXX": "S1"}}}
        self.assertFalse(sp.cooling(st, "IMPROVE", "P-00001")[0],
...
class TestRotate(unittest.TestCase):
    def test_length_is_always_preserved(self):
        ...
        st = {"history": hist(1), "offered": {"IMPROVE": {"P-00000": "S1", "P-00002": "S1"}}}
        out, n_cool = sp.rotate_candidates(rows(10), st, "IMPROVE", target=6)
    ...
    def test_cooled_rows_go_to_the_back(self):
        st = {"history": hist(1), "offered": {"IMPROVE": {"P-00000": "S1", "P-00001": "S1"}}}
        out, _ = sp.rotate_candidates(rows(10), st, "IMPROVE", target=6)
    ...
    def test_priority_head_survives_the_sample(self):
        ...
        out, _ = sp.rotate_candidates(rows(30), {"history": []}, "IMPROVE", target=21)
    ...
    def test_sample_reaches_past_the_head(self):
        ...
        out, _ = sp.rotate_candidates(rows(60), {"history": []}, "IMPROVE", target=21)
    ...
    def test_all_cooling_returns_plain_order(self):
        ...
        st = {"history": hist(1),
              "offered": {"IMPROVE": {f"P-{i:05d}": "S1" for i in range(5)}}}
        out, n_cool = sp.rotate_candidates(rows(5), st, "IMPROVE", target=3)
        self.assertEqual([r["id"] for r in out], [r["id"] for r in rows(5)])
        self.assertEqual(n_cool, 5)

    def test_empty_lane_is_safe(self):
        self.assertEqual(sp.rotate_candidates([], {"history": []}, "IMPROVE", 21), ([], 0))

    def test_stable_within_a_sitting(self):
        """The plan is run several times per iteration; the printed list must not move."""
        st = {"history": hist(1, 2)}
        a, _ = sp.rotate_candidates(rows(40), st, "IMPROVE", target=21)
        b, _ = sp.rotate_candidates(rows(40), st, "IMPROVE", target=21)
        self.assertEqual([r["id"] for r in a], [r["id"] for r in b])

    def test_resamples_once_an_observation_is_recorded(self):
        a, _ = sp.rotate_candidates(rows(40), {"history": hist(1, 2)}, "IMPROVE", 21)
        b, _ = sp.rotate_candidates(rows(40), {"history": hist(1, 2, 3)}, "IMPROVE", 21)
    ...
    def test_different_lanes_sample_differently(self):
        st = {"history": hist(1)}
        a, _ = sp.rotate_candidates(rows(40), st, "IMPROVE", 21)
...
class TestCoolKeyNamespacing(unittest.TestCase):
    ...
    def test_the_two_kinds_of_work_cool_INDEPENDENTLY(self):
        ...
        st = {"history": hist(1), "offered": {"IMPROVE": {"pid:P-00001": "S1"}}}
        self.assertTrue(sp.cooling(st, "IMPROVE", "pid:P-00001")[0])
        self.assertFalse(sp.cooling(st, "IMPROVE", "P-00001")[0])


class TestStampOnRecord(unittest.TestCase):
    def _pending(self, lane="IMPROVE"):
    ...
    def test_recording_the_drawn_lane_stamps_its_offers(self):
        st = sp.record(self._pending(), "IMPROVE", "hit", session=127, today="2026-08-01")
        self.assertEqual(st["offered"]["IMPROVE"], {"P-00001": "S127", "P-00002": "S127"})
    ...
    def test_working_a_DIFFERENT_lane_cools_nothing(self):
        ...
        # The draw offered IMPROVE rows; the session worked EXPAND instead. The two
        # lanes must DIFFER or this control proves nothing (it briefly did not, when
        # a blanket VERIFY->IMPROVE rename at the 02 AUG lane collapse made both
        # sides the same lane and the stamp fired correctly).
        st = sp.record(self._pending("IMPROVE"), "EXPAND", "hit",
                       session=127, today="2026-08-01")
        self.assertEqual(st.get("offered", {}).get("IMPROVE", {}), {})
    ...
    def test_no_pending_means_no_stamp(self):
        ...
        st = sp.record(st, "IMPROVE", "hit", session=127, today="2026-08-01")
    ...
    def test_legacy_pending_without_offered_is_safe(self):
        ...
        st = {"arms": {}, "history": [],
              "pending": {"date": "2026-08-01", "lane": "IMPROVE"}}
        st = sp.record(st, "IMPROVE", "hit", session=127, today="2026-08-01")
        self.assertIsNone(st["pending"])
        self.assertEqual(st.get("offered", {}).get("IMPROVE", {}), {})
